...
//         // WHERE s.name LIKE ?`,
//         // [`%${searchTerm}%`], 
//         "select s.sid,s.name,s.email,s.contact,c.name from student s join courses c on s.courseid=c.courseid where name like ? OR email like ? OR name like ?",[%${name}%,%${name}%,%${name}%],
//         (err, results) => {
...
app.get("/searchstudent", (req, res) => {
    ...
    let sql = `
        SELECT s.sid, s.name, s.email, s.contact, c.name as course_name
        FROM student s
        JOIN courses c ON s.courseid = c.courseid
        WHERE s.name LIKE ? OR s.email LIKE ? OR c.name LIKE ?
    `;

    db.query(sql, [likeTerm, likeTerm, likeTerm], (err, results) => {
        if (err) return res.status(500).send("Server Error");
        res.json(results);
    });
});
